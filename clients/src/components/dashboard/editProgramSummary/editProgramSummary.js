    ...
    return (
      <div key={measure.Measure_ID}>
        <p>
          ...
          {"At least " +
            measure.Percent_to_reach_target * 100 +
            "% of subjects score " +
            (measure.Value_Name
              ? "'" + measure.Value_Name + "'"
              : measure.Target_Score * 100 + "%") +
            " or higher on " +
            measure.Tool_Name +
            "."}
        </p>
...
export default class EditProgramSummary extends Component {
  ...
  componentDidMount() {
    ...
    axios
      .get("/api/rubric/getListWithScale/" + localStorage.getItem("Cycle_Id"))
      .then(res => {
        if (res.data.status) {
          this.setState({
            rubrics: res.data.rubrics,
            toolName: res.data.rubrics[0].Rubric_Title
              ? res.data.rubrics[0].Rubric_Title
              : null
          });
        }
      });
  ...
  handleDeleteOutcome(e) {
    let tempSummary = this.state.programSummary;
    let outcomeIndex = tempSummary.outcomes.findIndex(
      o => o.Outcome_ID === e.target.id
    );
  ...
  handleOutcomeChange(e) {
    ...
    let index = this.state.programSummary.outcomes.findIndex(
      o => o.Outcome_ID === id
    );
    let tempSummary = this.state.programSummary;
  ...
  handleAddRubricMeasure(e) {
    if (this.state.rubrics) {
      ...
      alert(
        "There are no rubrics associated with the current cycle. \n\nMigrate or create rubrics before making " +
          "rubric measures."
      );
    }
  }
  ...
  handleDeleteMeasure(e) {
    ...
    let measureIndex = tempSummary.outcomes[outcomeIndex].measures.findIndex(
      m => m.Measure_ID === e.target.id
    );
  ...
  addNewMeasure(e) {
    ...
    let rubricIndex = this.state.rubrics.findIndex(
      r => r.Rubric_Title === this.state.toolName
    );
    let valueName = null;
    let targetScore = this.state.targetScore;

    if (rubricIndex > -1) {
      valueName = this.state.rubrics[rubricIndex].scale[
    ...
    let index = this.state.programSummary.outcomes.findIndex(
      o => o.Outcome_ID === this.state.outcomeIdOfNewMeasure
    );
    let tempSummary = this.state.programSummary;
  ...
  handleSave() {
    axios
      .post(
        "/api/editProgramSummary/editProgramSummary",
        this.state.programSummary
      )
      .then(res => {
    ...
    if (this.state.deletedOutcomeIds.length > 0) {
      axios
        .post(
          "/api/editProgramSummary/deleteOutcomes",
          this.state.deletedOutcomeIds
        )
        .then(res => {});
    }
    ...
    if (this.state.deletedMeasureIds.length > 0) {
      axios
        .post(
          "/api/editProgramSummary/deleteMeasures",
          this.state.deletedMeasureIds
        )
        .then(res => {});
    }
    ...
    if (this.state.deletedCourseMappings.length > 0) {
      axios.post(
        "/api/curriculum/deleteOutcomeCurriculumMappings",
        this.state.deletedCourseMappings
      );
    }
  }
    ...
    if (
      ...
      return (
        <>
          ...
          {this.state.showAddRubricMeasurePopup ? (
            <AddRubricMeasurePopup
          ...
          {this.state.showAddTestMeasurePopup ? (
            <AddTestMeasurePopup
              closePopup={this.closePopup}
              submit={this.addNewMeasure}
              handleInputChange={this.handleInputChange}
